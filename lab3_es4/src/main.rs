...
impl<'a, T: Copy + PartialEq + Default> Reactor<'a, T> {
    ...
    pub fn create_compute<F: 'a + Fn(&[T]) -> T>(
        ...
        let compute_cell = ComputeCell {
            ...
            dependencies: dependencies.to_vec(),    //vettore di CellId da cui dipende il compute cell
    ...
    fn values(&self, cell_ids: &[CellId]) -> Result<Vec<T>, CellId> {
        ...
        cell_ids
            .iter()
            .map(|&CellId| self.value(CellId).ok_or(CellId))
            .collect()
    }
...
fn main() {
    ...
    println!("input   {:?}", CellId::Input(input));
    let output = reactor
