...
fn time_comparison(i1: Instant, i2: Instant) -> (Duration,TimeComparison){
    match i1.cmp(&i2){
        Ordering::Less => {
            return (i2.duration_since(i1), TimeComparison::Lesser)
        }
        Ordering::Equal => {
            return (i2.duration_since(i1), TimeComparison::Equal)
        }
        Ordering::Greater => {
            return (i1.duration_since(i2), TimeComparison::Greater)
        }
    }
}
    ...
    fn take(&self) -> Option<T> {
        loop {
            ...
            for element in lock.iter() {
                let current_difference = time_comparison(element.0, now);
                if nearest_element.is_none() || current_difference.0 < nearest_difference.clone().unwrap().0 {
...
//            take(&self) -> Option<T>: Cerca l'elemento t con scadenza più ravvicinata: se tale
//            scadenza è già stata oltrepassata, restituisce Some(t); se la scadenza non è ancora stata
//            superata, attende senza consumare cicli di CPU, che tale tempo trascorra, per poi restituire
//            Some(t); se non è presente nessun elemento in coda, restituisce None. Se, durante l'attesa,
//            avviene un cambiamento qualsiasi al contenuto della coda, ripete il procedimento suddetto
//            con il nuovo elemento a scadenza più ravvicinata (ammesso che ci sia ancora).
            if nearest_difference.clone().unwrap().1 == TimeComparison::Greater {
                lock = self.cv.wait_timeout_while(lock, nearest_difference.clone().unwrap().0, |l| { (*l).len() == queue_length }).unwrap().0;
